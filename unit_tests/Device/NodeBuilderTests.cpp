...
//   UA_StatusCode_name(statuscode); std::string type_name =
//   toString(&ua_type_name); UA_QualifiedName_clear(&ua_type_name);
...
// /**
//  * @brief Helper class for relating Information_Model::DataType to
//  UA_DataType
//  *
...
//     NameGenerator& names, const char*& spec, std::optional<std::string>
//     group) {
//   // precondition: *spec == '('
//   // postcondition: *spec == ')'
...
//     NOLINT(readability-magic-numbers) result_ = UA_Server_browse(ua_server_,
//     MAX_REFERENCES, &browse_description);
...
//     auto status = UA_Server_readNodeClass(ua_server, type_node,
//     &actual_class); EXPECT_EQ(status, UA_STATUSCODE_GOOD) <<
//     UA_StatusCode_name(status); EXPECT_EQ(actual_class, expected_class);
//   }
...
//           checkType(ref_desc.typeDefinition.nodeId,
//           UA_NODECLASS_VARIABLETYPE); children.ignore([](const
//           UA_ReferenceDescription& child) -> bool {
...
//           UA_StatusCode_name(status); EXPECT_EQ(value.type,
//           Types::UA_Type()); UA_Variant_clear(&value);
//         },
...
//           EXPECT_EQ(status, UA_STATUSCODE_GOOD) <<
//           UA_StatusCode_name(status); UA_Variant_clear(&value);
//         },
...
//    * are ignored. Only the subelements are checked. This is in contrast to
//    the
//    * other check* methods, each of which checks the full respective state.
//    */
//   /*
//    * In the overall procedure, the remaining aspects of DeviceElementGroup
//    are
//    * checked as part of checkDeviceElement, if the  DeviceElementGroup
//    appears
//    * as a specialization of a DeviceElement. If, on the other hand, the
//    * DeviceElementGroup appears as the result of
//    Device::getDeviceElementGroup,
//    * then there is nothing on the UA side to check against the missing
//    aspects.
//    */
...
//     Browse root_before(ua_server, UA_NODEID_NUMERIC(0,
//     UA_NS0ID_OBJECTSFOLDER)); root_before.ignore(
...
//     Browse root_after(ua_server, UA_NODEID_NUMERIC(0,
...
//             return UA_ExpandedNodeId_equal(&ref_desc.nodeId,
//             &old_ref.nodeId);
//           },
...
//     DataTypes<Information_Model::DataType::Unsigned_Integer,
//     UA_TYPES_UINT64>, DataTypes<Information_Model::DataType::Double,
//     UA_TYPES_DOUBLE>, DataTypes<Information_Model::DataType::Time,
//     UA_TYPES_DATETIME>, DataTypes<Information_Model::DataType::Opaque,
//     UA_TYPES_BYTESTRING>, DataTypes<Information_Model::DataType::String,
//     UA_TYPES_STRING>>;
