...
T getNodeDescription(const char *node_descriptor_name, std::string test_name)
{
    ...
    try
    {
        node_descriptor = nlohmann::setUpNodeDescriptors<T>(node_descriptor_name)[test_number];
    }
    catch (std::exception &ex)
    {
        std::cerr << "Cought a JSON exception for file: " << node_descriptor_name << "for test: " << test_name << " " << ex.what() << std::endl;
    }
...
std::vector<T> getNodeDescriptors(const char *node_descriptors_json_file)
{
    ...
    try
    {
        node_descriptors = nlohmann::setUpNodeDescriptors<T>(node_descriptors_json_file);
    }
    catch (std::exception &ex)
    {
        std::cerr << "Cought a JSON exception for file: " << node_descriptors_json_file << " " << ex.what() << std::endl;
    }
