...
constchar *baseNodeDescriptorInformation[MAX_BASE_NODE_DESCRITPOR_TEST_COUNT][BASE_NODE_DESCRIPTOR_FIELD_COUNT] = {
...
BaseNodeDescriptor setUpBaseNodeDescriptor(const char *nodeClassType, const char *writableFlag,
                                           const char *locale, const char *uniqueId, const char *displayName,
                                           const char *browseName, const char *description)
{
    BaseNodeDescriptor baseNodeDescritpor;
    baseNodeDescritpor.nodeClassType = identifyNodeClassType(nodeClassType);
...
TEST(NodeInformationTests, BaseNodeDescriptorContentFormatTest)
{
    ...
    {
        BaseNodeDescriptor baseNodeDescriptor = setUpBaseNodeDescriptor(
        ...
        EXPECT_EQ(identifyNodeClassType(baseNodeDescriptorInformation[i][0]), baseNodeDescriptor.nodeClassType);
