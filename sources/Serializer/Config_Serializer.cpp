...
shared_ptr<Logger> serializer_logger =
    LoggerRepository::getInstance().registerLoger(
        "Open62541 Config Serializer");
...
static void from_json(const json &j, OperationalLimits &p) {
  try {
    ...
    p.max_nodes_per_translate_browse_paths_to_nodeids =
        j.at("max_nodes_per_translate_browse_paths_to_nodeids")
            .get<UA_UInt32>();
...
static void from_json(const json &j, Config &p) {
  try {
    ...
    p.max_references_per_node =
        j.at("max_references_per_node").get<UA_UInt32>();
...
const Config open62541::deserializeConfig(const string &file_path) {
  ...
  if (input_file_stream) {
    ...
    serializer_logger->log(SeverityLevel::ERROR,
