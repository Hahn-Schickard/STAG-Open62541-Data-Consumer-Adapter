...
void NodeBuilder::historize(UA_NodeId node_id, const UA_DataType* type) {
  ...
  UA_NodeId NodeBuilder::addObjectNode(
    ...
    checkStatusCode("While creating Object Node for " + element->id() + " " +
            element->name(),
        status);
    ...
    try {
      ...
      device->visit([this, parent_id](const ElementPtr& element) {
        try {
          ...
          logger_->error(
              "Failed to create an Element Node {}:{} for Device Node "
              "{}. Status: {}",
  ...
  UA_StatusCode NodeBuilder::deleteDeviceNode(const string& device_id) {
    ...
    if (UA_StatusCode_isBad(result)) {
      ...
      logger_->trace(
          "Removed linked data sources for device node {}", device_id);
    }
  ...
  UA_VariableAttributes setValueAttributes(
    ...
    value_attributes.value.type = nullptr;
  ...
  UA_StatusCode NodeBuilder::addReadableNode(const MetaInfoPtr& meta_info,
    ...
    try {
      ...
      status = UA_Server_addDataSourceVariableNode(server_, node.id,
          node.parent, node.reference_type, node.name, type_definition,
          value_attributes, data_source, repo_.get(), nullptr);
  ...
  UA_StatusCode NodeBuilder::addWritableNode(const MetaInfoPtr& meta_info,
    ...
    try {
      ...
      auto value_attributes =
          setValueAttributes(meta_info, writable->dataType());
      value_attributes.accessLevel = UA_ACCESSLEVELMASK_WRITE;
      UA_DataSource data_source;
