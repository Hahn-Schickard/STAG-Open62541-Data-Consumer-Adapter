...
bool startServer()
{
  ...
  UA_ServerConfig_setDefault(UA_Server_getConfig(opcua_server));
...
OPEN62541_ReturnStatus addDeviceNode(Device *device)
{
  ...
  node_attr.description =
      UA_LOCALIZEDTEXT("EN_US", (char *)device->getElementDescription().c_str());
                                            ...
                                            UA_NodeId parent_id)
{
  ...
  case Group:
  {
    status = addGroupNode(static_cast<DeviceElementGroup *>(element), parent_id);
    break;
  }
  case Function:
  {
                                    ...
                                    UA_NodeId parent_id)
{
  ...
  UA_NodeId group_node_id = UA_NODEID_STRING(
      server_namespace_index, (char *)device_element_group->getElementRefId().c_str());
  UA_NodeId reference_type_id = UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES);
  UA_QualifiedName group_browse_name = UA_QUALIFIEDNAME(
      server_namespace_index, (char *)device_element_group->getElementName().c_str());
  UA_NodeId type_definition = UA_NODEID_NUMERIC(0, UA_NS0ID_BASEOBJECTTYPE);
  UA_ObjectAttributes node_attr = UA_ObjectAttributes_default;
  ...
  node_attr.displayName =
      UA_LOCALIZEDTEXT("EN_US", (char *)device_element_group->getElementName().c_str());
                                     ...
                                     UA_NodeId parent_id)
{
  ...
  node_attr.accessLevel =
      UA_ACCESSLEVELMASK_READ | UA_ACCESSLEVELMASK_WRITE; //@TODO: change to a
                                                          // function that assigns
                                                          // proper accessLevel
  node_attr.description =
      UA_LOCALIZEDTEXT("EN_US",
                       (char *)metric->getElementDescription().c_str());
  node_attr.displayName =
      UA_LOCALIZEDTEXT("EN_US",
                       (char *)metric->getElementName().c_str());
