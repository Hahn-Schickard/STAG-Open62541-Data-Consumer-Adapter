...
static UA_INLINE UA_DurationRange UA_DURATIONRANGE(UA_Duration min,
                                                   UA_Duration max) {
...
static UA_StatusCode createEndpoint(UA_ServerConfig *conf,
  ...
  UA_StatusCode retval =
      UA_Array_copy(conf->accessControl.userTokenPolicies,
...
static UA_StatusCode setDefaultConfig(UA_ServerConfig *conf) {
  ...
  conf->applicationDescription.applicationUri =
      UA_STRING_ALLOC(APPLICATION_URI_SERVER);
...
UA_EXPORT UA_StatusCode HS_ServerConfig_setBasics(UA_ServerConfig *conf) {
  return setDefaultConfig(conf);
}

static UA_StatusCode addDefaultNetworkLayers(UA_ServerConfig *conf,
                                             UA_UInt16 portNumber,
                                             UA_UInt32 sendBufferSize,
                                             UA_UInt32 recvBufferSize) {
  return HS_ServerConfig_addNetworkLayerTCP(conf, portNumber, sendBufferSize,
                                            recvBufferSize);
}

UA_EXPORT UA_StatusCode HS_ServerConfig_addNetworkLayerTCP(
    UA_ServerConfig *conf, UA_UInt16 portNumber, UA_UInt32 sendBufferSize,
    UA_UInt32 recvBufferSize) {
  /* Add a network layer */
...
UA_EXPORT UA_StatusCode HS_ServerConfig_addSecurityPolicyNone(
    UA_ServerConfig *config, const UA_ByteString *certificate) {
  UA_StatusCode retval;
  ...
  retval = UA_SecurityPolicy_None(
      &config->securityPolicies[config->securityPoliciesSize], NULL,
...
UA_EXPORT UA_StatusCode HS_ServerConfig_addEndpoint(
    UA_ServerConfig *config, const UA_String securityPolicyUri,
    UA_MessageSecurityMode securityMode) {
  UA_StatusCode retval;
...
HS_ServerConfig_addAllEndpoints(UA_ServerConfig *config) {
  UA_StatusCode retval;
...
UA_EXPORT UA_StatusCode HS_ServerConfig_setMinimalCustomBuffer(
    UA_ServerConfig *config, UA_UInt16 portNumber,
    const UA_ByteString *certificate, UA_UInt32 sendBufferSize,
    UA_UInt32 recvBufferSize) {
  if (!config)
    return UA_STATUSCODE_BADINVALIDARGUMENT;
  ...
  retval = addDefaultNetworkLayers(config, portNumber, sendBufferSize,
                                   recvBufferSize);
  if (retval != UA_STATUSCODE_GOOD) {
    UA_ServerConfig_clean(config);
    return retval;
  ...
  /* Allocate the SecurityPolicies */
  retval = HS_ServerConfig_addSecurityPolicyNone(config, certificate);
  if (retval != UA_STATUSCODE_GOOD) {
    UA_ServerConfig_clean(config);
    return retval;
  }
  ...
  /* Allocate the endpoint */
  retval = HS_ServerConfig_addEndpoint(config, UA_SECURITY_POLICY_NONE_URI,
                                       UA_MESSAGESECURITYMODE_NONE);
  if (retval != UA_STATUSCODE_GOOD) {
    UA_ServerConfig_clean(config);
    return retval;
...
UA_EXPORT UA_StatusCode UA_ServerConfig_addSecurityPolicyBasic128Rsa15(
    UA_ServerConfig *config, const UA_ByteString *certificate,
...
UA_EXPORT UA_StatusCode UA_ServerConfig_addSecurityPolicyBasic256(
    UA_ServerConfig *config, const UA_ByteString *certificate,
    const UA_ByteString *privateKey) {
  UA_StatusCode retval;
...
UA_EXPORT UA_StatusCode UA_ServerConfig_addSecurityPolicyBasic256Sha256(
    UA_ServerConfig *config, const UA_ByteString *certificate,
    const UA_ByteString *privateKey) {
  UA_StatusCode retval;
...
UA_EXPORT UA_StatusCode UA_ServerConfig_addAllSecurityPolicies(
    UA_ServerConfig *config, const UA_ByteString *certificate,
    const UA_ByteString *privateKey) {
  UA_StatusCode retval;
...
UA_EXPORT UA_StatusCode UA_ServerConfig_setDefaultWithSecurityPolicies(
    UA_ServerConfig *conf, UA_UInt16 portNumber,
  ...
  retval =
      UA_ServerConfig_addAllSecurityPolicies(conf, certificate, privateKey);
  if (retval != UA_STATUSCODE_GOOD) {
    UA_ServerConfig_clean(conf);
    return retval;
  }
  ...
  retval = HS_ServerConfig_addAllEndpoints(conf);
  if (retval != UA_STATUSCODE_GOOD) {
    UA_ServerConfig_clean(conf);
    return retval;
  }
...
static UA_INLINE void UA_ClientConnectionTCP_poll_callback(UA_Client *client,
                                                           void *data) {
...
UA_StatusCode UA_ClientConfig_setDefault(UA_ClientConfig *config) {
  ...
  if (config->securityPoliciesSize > 0) {
    UA_LOG_ERROR(
        &config->logger, UA_LOGCATEGORY_NETWORK,
  ...
  UA_StatusCode retval = UA_SecurityPolicy_None(
      config->securityPolicies, NULL, UA_BYTESTRING_NULL, &config->logger);
  if (retval != UA_STATUSCODE_GOOD) {
...
#ifdef UA_ENABLE_ENCRYPTION
UA_StatusCode UA_ClientConfig_setDefaultEncryption(
    UA_ClientConfig *config, UA_ByteString localCertificate,
  ...
  retval = UA_CertificateVerification_Trustlist(
      &config->certificateVerification, trustList, trustListSize, NULL, 0,
  ...
  retval = UA_SecurityPolicy_Basic128Rsa15(
      &config->securityPolicies[1], &config->certificateVerification,
  ...
  retval = UA_SecurityPolicy_Basic256(
      &config->securityPolicies[2], &config->certificateVerification,
      localCertificate, privateKey, &config->logger);
  if (retval != UA_STATUSCODE_GOOD)
    return retval;
  ++config->securityPoliciesSize;
  ...
  retval = UA_SecurityPolicy_Basic256Sha256(
      &config->securityPolicies[3], &config->certificateVerification,
      localCertificate, privateKey, &config->logger);
  if (retval != UA_STATUSCODE_GOOD)
    return retval;
  ++config->securityPoliciesSize;
